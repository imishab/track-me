...
export function CategoryAnalytics({ categoryName, habits }: CategoryAnalyticsProps) {
  ...
  const stats = useMemo(() => {
    ...
    if (totalHabits === 0) {
      return {
        totalHabits: 0,
        completedTodayPercent: 0,
        completionRate: 0,
        chartData: [] as { date: string; fullDate: string; rate: number }[],
        habitChartData: [] as { name: string; fullName: string; rate: number }[],
        streak: 0,
      }
    }
    ...
    const completedTodayPercent = Math.round((todayRatioSum / totalHabits) * 100)
            ...
            <CardContent>
              {stats.chartData.length === 0 ? (
                <p className="text-sm text-muted-foreground py-8 text-center">
                  No data yet. Complete habits in Todo to see trends.
                </p>
              ) : (
